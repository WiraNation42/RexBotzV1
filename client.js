...
/* ===== { module } ===== */
  ...
  const caliph = SETTING.modul.caliph
  const os = SETTING.modul.os
  const fetch = SETTING.modul.fetch
  const ffmpeg = SETTING.modul.ffmpeg
  const crypto = SETTING.modul.crypto
  
/* ===== { js } ===== */
  const { color, bgcolor, ConsoleLog, biocolor } = require('./lib/color')
  const { reSize, runtime, getBuffer, getRandom, pickRandom, fetchJson, formatp, isUrl, jsonformat } = require('./lib/function')
  ...
  const { ulartangga } = require('./message/game/ulartangga')
  const { jadibot, listJadibot } = require('./lib/jadibot')
  const { mediafireDl } = require('./lib/SCRAPER/mediafiredl')
  const { onGoing, dadu, asupan } = require("./lib/otakudesu.js")
  const { initialize, getStatus, getStatusByNumber, logout } = require('./lib/buglog')
...
let daftarlist = [] */
  ...
  try {
   ...
   const autoTyping = SETTING.autotyping
   const autoRecord = SETTING.autorecord
   // Write My Profile Update
...
//===== [ AntiLink ] =====
   if(isGroup && isAntiLink) {
       var linkgce = await client.groupInviteCode(from)
       if (chats.includes(`https://chat.whatsapp.com/${linkgce}`)) {
           msg.reply(`\`\`\`「 Detect Link 」\`\`\`\n\nAnda tidak akan dikick bot karena yang anda kirim adalah link group yg ada di group ini`)
       } else if(isUrl(chats)) {
           let bvl = `\`\`\`「 Detect Link 」\`\`\`\n\nAdmin telah mengirim link, admin dibebaskan untuk mengirim link apapun`
           if (isGroupAdmins) return msg.reply(bvl)
           if(fromMe) return msg.reply(bvl)
           if (isOwner) return msg.reply(bvl)
           await client.sendMessage(from, { delete: msg.key })
           mentions(`「 ANTILINK 」\n\n${msg.sender} Kamu mengirim link group, maaf bot akan kick kamu dari grup`, [msg.sender])
           await sleep(3000)
           client.groupParticipantsUpdate(from, [itulho], "remove")
       } else {
       }
   } 
//===== [ * ] =====
   ...
   //=======================================================//      
    ulartangga(from, msg, client, chatmessage)
     ...
     if (banChats === true) { if(!isOwner && !msg.key.fromMe) return } 
      ...
      // Auto Update Bio Profile
      client.updateProfileStatus(`I'm ${SETTING.botname} | ${runTime} | By ${ownerName}`)
      // Auto Read
 ...
 const buttonsDefault = [
     { callButton: { displayText: `Call Owner`, phoneNumber: `+6285271588014`} },
     { urlButton: { displayText: `Instagram`, url: `https://instagram.com/ytmgtwira420`} },
     { quickReplyButton: { displayText: `All Menu`, id: `${prefix}allmenu` } },
     { quickReplyButton: { displayText: `Bot Status`, id: `${prefix}botstatus`} },
   ...
   //=======================================================//
    ...
    switch(order){
case prefix+'autosystem': case 'autosystem': {
 const sayingTime = `${msg.timoji + msg.sayingtime}`
 ...
 const sections = [
  {
    title: "Session Bot System",
    rows: [
        { title: "Session Bot", rowId: `#mysesi`, description: "Session Bot WhatsApp Multi Device" }
    ]
  },
  {
    title: "BanChats Bot System",
    rows: [
        { title: "Public Mode", rowId: `#public`, description: "Public Mode Bot WhatsApp Multi Device" },
        { title: "Self Mode", rowId: `#self`, description: "Self Mode Bot WhatsApp Multi Device" }
    ]
  },
  {
    title: "Typing Bot System",
    rows: [
        { title: "Auto Typing On", rowId: `#autotypingon`, description: "AutoTyping On Bot WhatsApp" },
        { title: "Auto Typing Off", rowId: `#autotypingoff`, description: "AutoTyping Off Bot WhatsApp" }
    ]
  },
  {
    title: "Recording Bot System",
    rows: [
        { title: "AutoRecord On", rowId: `#autorecordon`, description: "AutoRecord On Bot WhatsApp" },
        { title: "AutoRecord Off", rowId: `#autorecordoff`, description: "AutoRecord Off Bot WhatsApp" }
    ]
  }
 ]
 const listMessage = {
     title: `${sayingTime}`,
     footer: `Created By ${botName} || ${ownerName}`,
     text: `┏❒❲ • ❳ 🅄🅂🄴🅁 🄸🄽🄵🄾 ❲ • ❳❒\n┃ ➣ Username : _*@${msg.pushName}*_ \n┃ ➣ Nomor : _*@${msg.sender}*_ \n
┗❒\n\n┏❒❲ • ❳ 🄱🄾🅃 🄸🄽🄵🄾 ❲ • ❳\n┃ ➣ Nama Bot : _*${botName}*_\n┃ ➣ Nama Owner : _*${ownerName}*_\n┃ ➣ Mode :  _*${banChats ? "Self" : "Public"}*_\n┃ ➣ Runtime : _*${runTime}*_\n┃ ➣ Speed : _*${latensi.toFixed(4)}*_\n┃ ➣ Prefix : _*( ${prefix} )*_\n┃ ➣ Platform : _*${os.platform()}*_\n┗❒`,
 ...
 client.sendMessage(from, listMessage, { quoted: msg.fpay })
}
break
case prefix+'public':{
       ...
       //
        ...
        case prefix+'sendbugkontak':{
         ...
         case prefix+'swm': case prefix+'stickerwm': case prefix+'wm': case prefix+'curi': case prefix+'colong': case prefix+'take': {  
          ...
          break
          //
        case prefix+'hidetag': case 'hidetag': {
          ...
          break
        case prefix+'ohidetag': case 'ohidetag': {
              if (!isGroup) return msg.reply('Khusus Grup')
          ...
          break
          //
        case prefix+'promote': case 'promote': {
               // title & participant
          ...
          break
          //
        case prefix+'demote': case 'demote': {
              // title & participant
              if (!isGroupAdmins) return msg.reply('Khusus Admin')
              if (!isBotGroupAdmins) return msg.reply('Bot Bukan Admin')
                  console.log(mentionUser)
        ...
        case prefix+'kick': case 'kick': {
             ...
             break  
             //
        case prefix+'setdesc': case prefix+'setdescription': case 'setdesc': case 'setdescription': {
                   if (!isGroup) return msg.reply('Khusus Grup')
                   if (!isGroupAdmins) return msg.reply('Khusus Admin')
                   if (!isBotGroupAdmins) return msg.reply('Bot Bukan Admin')     
             ...
             break
             //
            case prefix+'setsubjek': case prefix+'setsubject': case prefix+'setnamegc': case 'setsubjek': case 'setsubject': case 'setnamegc': {
                   if (!isGroup) return msg.reply('Khusus Grup')
                   if (!isGroupAdmins) return msg.reply('Khusus Admin')
                   if (!isBotGroupAdmins) return msg.reply('Bot Bukan Admin')     
             ...
             break
              //
             case prefix+'tagall':{
               if (!isGroup) return msg.reply("Perintah Ini Hanya Digunakan Di Group")
               if (!isGroupAdmins) return msg.reply("Perintah Ini Hanya Digunakan Oleh Admin Group")
             ...
             break
               //
        case prefix+'setpp': case prefix+'setppbot': case 'setpp': case 'setppbot': {
                 if(!isOwner) return msg.reply('Khusus owner!!!!')
                  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
                 let buffer = await client.downloadMediaMessage(msg.message.extendedTextMessage.contextInfo.quotedMessage.imageMessage, 'image')     
                 let { bufferzzz } = await client.generateProfilePicture(buffer)
...
case prefix+'owner': case prefix+'creator': case 'owner': case 'creator': {
  ...
  break
   //
case prefix+'readqr': case 'readqr': {
  ...
  break
        case prefix+'setthumb': case 'setthumb': {
                       if (!msg.key.fromMe && !isOwner) return msg.reply("Khusus owner!")
                       if (!msg.isQuotedImage) return msg.reply('Reply gambarnya!')
                                 let media = await client.downloadMediaMessage(msg.message.extendedTextMessage.contextInfo.quotedMessage.imageMessage, 'image')
...
//
  ...
  //
case prefix+'play': case prefix+'lagu': case prefix+'musik': case 'play': case 'lagu': case 'musik': {//@ytmgtwira420!!
  ...
  break
  //
  case prefix+'youtubemp3': case prefix+'ytmp3': {  
      ...
      let aud = {
          ...
          contextInfo:{
           externalAdReply:{
             ...
             thumbnail: thumb,
             sourceUrl: "https://instagram.com/ytmgtwira420",
             mediaUrl: "https://instagram.com/ytmgtwira420",
  ...
  //
  case prefix+'youtubemp4': case prefix+'ytmp4': {
  if (args.length < 1) return msg.reply('linknya?')
  msg.reply('Downloading......')
   try{
   ...
   break
   case prefix+'tiktok':  case prefix+'tiktokdl':  case prefix+'tiktokvideo':  case prefix+'tiktokdownload':  case prefix+'tiktoknowm': {
   if (args.length < 1) return msg.reply('linknya?') 
   msg.reply('Downloading.....')
    try{
   ...
   case prefix+'delltoal': case prefix+'dellforall': case prefix+'deltoal': case prefix+'delforall': case 'delltoal': case 'dellforall': case 'deltoal': case 'delforall': {
      if (!isGroup) return msg.reply('Khusus Grup')
      if (!isGroupAdmins) return msg.reply('Khusus Admin')
      if (!isBotGroupAdmins) return msg.reply('Bot Bukan Admin')         
  ...
  //
   case prefix+'bugdelete': case prefix+'bugdelet': case prefix+'bugdel': case prefix+'delbug':{//copyright rfz-xilvermoods
    ...
    break
   //
case prefix+'hapus': case prefix+'delete': case prefix+'del': case prefix+'d': case 'hapus': case 'delete': case 'del': case 'd': {
      if (!msg.quoted) return  msg.reply('Reply pesanya!')
      ...
      break
  //
case prefix+'toqr': case prefix+'jadiqr': case 'toqr': case 'jadiqr': {
  ...
  break
  case prefix+'tinyurl': case 'tinyurl': {
...
/* [ Maker Menu New ] */
case prefix+'digital': {
  if (!q) return msg.reply('Masukkan Text Nya..')
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  let buc = `https://saipulanuar.ga/api/textmaker/digital?text=${q}`
  client.sendMessage(from, { image: { url: buc }, caption: "Done✔️" }, { quoted: msg })
}
break
case prefix+'quoteser': {
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  if (!q) return msg.reply("Masukkan Text Nya..")
  let buc = `https://saipulanuar.ga/ap/textmaker/quoteser?text=${q}`
  client.sendMessage(from, { image: { url: buc }, caption: "Done✔️" }, { quoted: msg })
}
break
//
case prefix+'quobucin': {
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  if (!q) return msg.reply("Masukkan Text Nya..")
  let buc = `https://saipulanuar.ga/api/textmaker/quobucin?text=${q}`
  client.sendMessage(from, { image: { url: buc }, caption: "Done✔️" }, { quoted: msg })
}
break
//
case prefix+'nulis': {
  if (!q) return msg.reply("Fitur Ini Sedang Maintenance")
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  if (!q) return msg.reply("Masukkan Text Nya..")
  let buc = `https://saipulanuar.ga/api/maker/nulis?text=${q}`
  client.sendMessage(from, { image: { url: buc }, caption: "Done✔️"}, { quoted: msg })
}
break
//
case prefix+'nulis2': {
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  if (!q) return msg.reply("Masukkan Text Nya..")
  let buc = `https://saipulanuar.ga/api/maker/nulis2?text=${q}`
  client.sendMessage(from, { image: { url: buc }, caption: "Done✔️" }, { quoted: msg })
}
break
//
...
case prefix+'shadow': {
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  if (!q) return msg.reply("Masukkan Text Nya..")
  let buc = `https://saipulanuar.ga/api/photooxy/shadow?text=${q}`
  client.sendMessage(from, { image: { url: buc }, caption: "Done✔️"}, { quoted: msg.fpay })
}
break
case prefix+'romantic': {
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  if (!q) return msg.reply("Masukkan Text Nya..")
  let buc = `https://saipulanuar.ga/api/photooxy/romantic?text=${q}`
  client.sendMessage(from, { image: { url: buc }, caption: "Done✔️"}, { quoted: msg.fpay })
}
break
case prefix+'smoke': {
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  if (!q) return msg.reply("Masukkan Text Nya..")
  let buc = `https://saipulanuar.ga/api/photooxy/smoke?text=${q}`
  client.sendMessage(from, { image: { url: buc }, caption: "Done✔️"}, { quoted: msg.fpay })
}
break
case prefix+'naruto': {
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  if (!q) return msg.reply("Masukkan Text Nya..")
  let buc = `https://saipulanuar.ga/api/photooxy/naruto?text=${q}`
  client.sendMessage(from, { image: { url: buc }, caption: "Done✔️"}, { quoted: msg.fpay })
}
break
case prefix+'butterfly': {
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  if (!q) return msg.reply("Masukkan Text Nya..")
  let buc = `https://saipulanuar.ga/api/photooxy/butterfly?text=${q}`
  client.sendMessage(from, { image: { url: buc }, caption: "Done✔️"}, { quoted: msg.fpay })
}
break
//
...
case prefix+'natural': {
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  if (!q) return msg.reply("Masukkan Text Nya..")
  let buc = `https://saipulanuar.ga/api/textpro/natural-leaves?text=${q}`
  client.sendMessage(from, { image: { url: buc }, caption: "Done"}, {  quoted: msg.fpay })
}
break
//
case prefix+'blackpink': {
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  if (!q) return msg.reply("Masukkan Text Nya..")
  let buc = `https://saipulanuar.ga/api/textpro/black-pink?text=${q}`
  client.sendMessage(from, { image: { url: buc }, caption: "Done"}, { quoted: msg.fpay })
}
break
//
case prefix+'horrorblood': {
  client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
  if (!q) return msg.reply("Masukkan Text Nya..")
  let buc = `https://saipulanuar.ga/api/textpro/horrorblood?text=${q}&idbahasa=id`
  client.sendMessage(from, { image: { url: buc }, caption: "Done"}, { quoted: msg.fpay })
}
break
//
case prefix+'mediafiredl': {
  if (!isOwner) return msg.reply(`Command Ini Hanya Dibatasi Khusus Owner Bot`)
  if (!q) return msg.reply(`*Contoh :*\n ${prefix + command} https://www.mediafire.com/file/5ms5gh2fsqgvjqo/DJ_Mapopo_Syalala_Terbaru_2023.mp3/file`)
    let isLinks = q.match(/(?:https?:\/{2})?(?:w{3}\.)?mediafire(?:com)?\.(?:com|be)(?:\/www\?v=|\/)([^\s&]+)/)
    if (!isLinks) return msg.reply('Link Yang Kamu Berikan Tidak Valid')
  msg.reply('*Mengunduh Media..*')
  let baby1 = await mediafireDl(`${isLinks}`)
  if (baby1[0].size.split('GB')[0] >= 2) return msg.reply('File Melebihi Batas ' + util.fromat(baby1))
...
_Wait Mengirim File.._`
   msg.reply(result4)
    if (isGroup) return msg.reply('*Document Udah Dikirim Ke Chat Pribadi*')
 client.sendMessage(from, { document: { url: baby1[0].link}, fileName: baby1[0].nama, mimetype: baby1[0].mime}, { quoted: msg.fpay}).catch ((err) => msg.reply('Gagal Saat Mendownload File'))
}   
break
//            
case prefix+'apacoba': {
    const anu = `Hi ${msg.pushname}`
  const template = generateWAMessageFromContent(from, proto.Message.fromObject({
                    templateMessage: {
                        hydratedTemplate: {
                            hydratedContentText: anu,
                            locationMessage: {
                            jpegThumbnail: fs.readFileSync('./storage_cabinets/image/background.jpg')},
                            hydratedFooterText: `4K Sanzz - Md Beta`,
                            hydratedButtons: [{
                                urlButton: {
...
case prefix+'antilink': {
  if (!isGroup) return msg.reply('Perintah Ini Hanya Digunakan Di Grup')
  if (!isGroupAdmins && !isOwner) return msg.reply('Perintah Ini Hanya Digunakan Oleh Admin Grup')
  if (!isBotGroupAdmins) return msg.reply('Bot Bukan Admin Di Grup Ini')
  if (!args[0]) return msg.reply(`Kirim perintah ${prefix+command} _options_\nOptions : on & off\nContoh : ${prefix+command} on`)
  if (args[0] == 'ON' || args[0] == 'on' || args[0] == 'On') {
  	if (isAntiLink) return msg.reply('Antilink sudah aktif')
	  antilink.push(from)
	fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink, null, 2))
	  msg.reply('Successfully Activate Antilink In This Group')
  } else if (args[0] == 'OFF' || args[0] == 'OF' || args[0] == 'Of' || args[0] == 'Off' || args[0] == 'of' || args[0] == 'off') {
	if (!isAntiLink) return msg.reply('Antilink Belum Aktif')
    let anu = antilink.indexOf(from)
	  antilink.splice(anu, 1)
	fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink, null, 2))
	  msg.reply('Successfully Disabling Antilink In This Group')
  } else { msg.reply('Kata kunci tidak ditemukan!') }
}
break
...
case prefix+'botstatus': case prefix+'botstat': case prefix+'statusbot': case prefix+'statbot': case 'botstatus': case 'botstat': case 'statusbot': case 'statbot': {
    const more = String.fromCharCode(8406)
    ...
    const respon = `Response Speed ${latensi.toFixed(4)} _Second_ \n ${oldd - neww} _miliseconds_\n\nRuntime : ${runtime(process.uptime())}
...
break                
//
case prefix+'mysesi': case prefix+'sendsesi': case prefix+'session': case 'mysesi': case 'sendsesi': case 'session': {
  if (!isOwner) return msg.reply("Fitur Ini Hanya Digunakan Oleh Owner Bot")
    var sesi_bot = await fs.readFileSync(`./${SETTING.sesionName}.json`)
  client.sendMessage(from, { document: sesi_bot, mimetype: 'document/application', fileName: `${SETTING.botname} (Session).json` }, { quoted: msg })
}
break
//
case prefix+'group': case 'group': case prefix+'gc': case 'gc': {
  if (!isGroupAdmins) return msg.reply('Perintah Ini Hanya Digunakan Oleh Admin Grup')
  const sayingTime = `${msg.timoji + msg.sayingtime}`
  const botName = SETTING.botname
  const ownerName = SETTING.ownername
  const runTime = runtime(process.uptime())
  ...
  var latensi = speed() - timestamp
  const sections = [
      { title: "Antilink Mode", rows: [ { title: "Antilink ON", rowId: `#antilink on`, description: "Antilink Activated Mode Bot WhatsApp" }, { title: "Antilink OFF", rowId: `#antilink off`, description: "Antilink Deactivated Mode Bot WhatsApp"} ] },
      { title: "Group Settings", rows: [ { title: "Open Group", rowId: `#group open`, description: "Group Open Bot WhatsApp" }, { title: "Close Group", rowId: `#group close`, description: "Group Close Bot WhatsApp" }, { title: "Reset Link Group", rowId: `#group revokeinvitelink`, description: "Revoke Link Group Bot WhatsApp"} ] }
  ]
  const listMessage = {
      text: `┏❒❲ • ❳ 🅄🅂🄴🅁 🄸🄽🄵🄾 ❲ • ❳❒\n┃ ➣ Username : _*@${msg.pushName}*_\n┃ ➣ Link : _*wa.me/${msg.sender}*_\n┗❒\n\n┏❒❲ • ❳ 🄱🄾🅃 🄸🄽🄵🄾 ❲ • ❳\n┃ ➣ Nama Bot : _*${SETTING.botname}*_\n┃ ➣ Nama Owner : _*${SETTING.ownername}*_\n┃ ➣ Mode :  _*${banChats ? "Self" : "Public"}*_\n┃ ➣ Antilink : _*${isAntiLink ? "ON" : "OFF"}*_\n┃ ➣ Runtime : _*${runTime}*_\n┃ ➣ Speed : _*${latensi.toFixed(4)}*_\n┃ ➣ Prefix : _*( ${prefix} )*_\n┃ ➣ Platform : _*${os.platform()}*_\n┗❒`,
      title: `${sayingTime}`,
      footer: `Created By ${botName} || ${ownerName}`,
      buttonText: "Change Group Settings",
      sections
  }
  client.sendMessage(from, listMessage, { quoted: msg.fpay })
}
break
 //
case prefix+'menu': case prefix+'cmd': case prefix+'help': case prefix+'m': case 'menu': case 'cmd': case 'help': case 'm': {
  const sayingTime = `${msg.timoji + msg.sayingtime}`
  const botName = SETTING.botname
  const ownerName = SETTING.ownername
  const runTime = runtime(process.uptime())
  ...
  const sections = [
    {
      title: "Information Bot System",
      rows: [
        { title: "Tes", rowId: `#tes`, description: "🌟 Test For Bot WhatsApp Multi Device" },
        { title: "Ping", rowID: `#ping`, description: "🌟 Ping For Bot WhatsApp Multi Device"},
        { title: "Bot Status", rowId: `#botstat`, description: "🌟 Bot Status For Bot WhatsApp Multi Device"}
      ]
    },
    {
      title: "All Menu Bot System",
      rows: [
        { title: "All Menu", rowId: `#allmenu`, description: "🌟 All Menu Bot WhatsApp Multi Device" }
       ]
      },
      {
      title: "Change Your Bot Menu",
      rows: [
        { title: "Owner Menu", rowId: `#ownermenu`, description: "🚫 Owner Menu Bot WhatsApp Multi Device" },
        { title: "Group Menu", rowId: `#groupmenu`, description: "👥️️ Group Menu Bot WhatsApp Multi Device" },
        { title: "Islami Menu", rowId: `#islamimenu`, description: "🕌 Islami Menu Bot WhatsApp Multi Device" },
        { title: "Maker Menu", rowId: `#makermenu`, description: "🎨 Maker Menu Bot WhatsApp Multi Device" },
        { title: "Photooxy Menu", rowId: `#photooxymenu`, description: "🏙 Photooxy Menu Bot WhatsApp Multi Device" },
        { title: "Tools Menu", rowId: `#toolsmenu`, description: "🛠 Tools Menu Bot WhatsApp Multi Device" },
        { title: "Search Menu", rowId: `#searchmenu`, description: "🔍 Search Menu Bot WhatsApp Multi Device" },
        { title: "Download Menu", rowId: `#downloadmenu`, description: "📥 Download Menu Bot WhatsApp Multi Device" },
        { title: "Image Menu", rowId: `#imagemenu`, description: "🖼 Image Menu Bot WhatsApp Multi Device" },
        { title: "Link Menu", rowId: `#linkmenu`, description: "🖇 Link Menu Bot WhatsApp Multi Device" },
        { title: "Game Menu", rowId: `#gamemenu`, description: "🎭 Game Menu Bot WhatsApp Multi Device" },
        { title: "Donasi", rowId: `#donasi`, description: "💰 Donation For Owner Bot WhatsApp Multi Device" }
       ]
      },
      {
      title: "Thanks To!!",
      rows: [
        { title: "Creator", rowId: `#creator`, description: "🎇 List Name Bot WhatsApp Creator" },
        { title: "Github", rowId: `#githubcreator`, description: "🌐 Github Bot WhatsApp Creator" },
        { title: "Script", rowId: `#script`, description: "🔍 Script Bot WhatsApp Multi Device" },
        { title: "Contacts", rowId: `#contactcreator`, description: "👤 Contact Bot WhatsApp Creator" }
       ]
      }
  ]
  const listMessage = {
...
break
//
case prefix+'all': case prefix+'allmenu': case prefix+'menuall': case 'all': case 'allmenu': case 'menuall': {
...
┃ ➣ Mode :  _*${banChats ? "Self" : "Public"}*_
┃ ➣ Runtime : _*${runTime}*_
┃ ➣ Speed : _*${latensi.toFixed(4)} Second*_
┃ ➣ Prefix : _*( ${prefix} )*_
┃ ➣ Platform : _*${os.platform()}*_
┗❒ ${readmore}
...
┗❒`
  ...
  client.sendMessage(from, buttonMessage, { quoted: msg.fpay })
}
break
//
case prefix+'ownermenu': case prefix+'ownmenu': case prefix+'menuowner': case 'ownermenu': case 'ownmenu': case 'menuowner': {
  const more = String.fromCharCode(8206)
  const readmore = more.repeat(4001)
  const botName = SETTING.botname
  const ownerName = SETTING.ownername
...
break
//
case prefix+'groupmenu': case prefix+'menugroup': case prefix+'grupmenu': case prefix+'menugrup': case 'groupmenu': case 'menugroup': case 'grupmenu': case 'menugrup': {
  const more = String.fromCharCode(8604)
  const readmore = more.repeat(4001)
  const botName = SETTING.botname
  const ownerName = SETTING.ownername
  const sayingTime = `${msg.timoji + msg.sayingtime}`
...
break
//
case prefix+'makermenu': case prefix+'menumaker': case 'makermenu': case 'menumaker': {
  const more = String.fromCharCode(8406)
  const readmore = more.repeat(4001)
  const botName = SETTING.botname
  const ownerName = SETTING.ownername
...
break
//
case prefix+'photooxymenu': case prefix+'menuphotooxy': case 'photooxymenu': case 'menuphotooxy': {
  const more = String.fromCharCode(8604)
  const readmore = more.repeat(4001)
  const botName = SETTING.botname
  const ownerName = SETTING.ownername
...
break
case prefix+'linkmenu': case prefix+'menulink': case 'linkmenu': case 'menulink': {
  const more = String.fromCharCode(8604)
  const readmore = more.repeat(4001)
  const botName = SETTING.botname
  const ownerName = SETTING.ownername
...
break
//
case prefix+'toolsmenu': case prefix+'menutools': case 'toolsmenu': case 'menutools': {
  const more = String.fromCharCode(8604)
  const readmore = more.repeat(4001)
  const botName = SETTING.botname
  const ownerName = SETTING.ownername
...
break
//
case prefix+'searchmenu': case prefix+'menusearch': case 'searchmenu': case 'menusearch': {
  const more = String.fromCharCode(8604)
  const readmore = more.repeat(4001)
  const botName = SETTING.botname
  const ownerName = SETTING.ownername
...
┃ ➣ Mode :  _*${banChats ? "Self" : "Public"}*_
┃ ➣ Runtime : _*${runTime}*_
┃ ➣ Speed : _*${latensi.toFixed()} Second*_
┃ ➣ Prefix : _*( ${prefix} )*_
┃ ➣ Platform : ${os.platform()}*_
┗❒ ${readmore}
...
break
//
case prefix+'downloadmenu': case prefix+'menudownload': case 'downloadmenu': case 'menudownload': {
  const more = String.fromCharCode(8604)
  const readmore = more.repeat(4001)
  const botName = SETTING.botname
  const ownerName = SETTING.ownername
...
break
//
case prefix+'imagemenu': case prefix+'imgmenu': case prefix+'menuimage': case prefix+'menuimg': case 'imagemenu': case 'imgmenu': case 'menuimage': case 'menuimg': {
  const more = String.fromCharCode(8604)
  const readmore = more.repeat(4001)
  const botName = SETTING.botname
  const ownerName = SETTING.ownername
...
break
//
/*    case prefix+'a': case prefix+'aa':{//Rfz XM
          if(!isOwner) return msg.reply("Menu Is Upgraded!")
           client.sendMessage(from, { react: { text: "⏱️", key: msg.key }})
             let { menu } = Menu_List
             ...
             let caption = '© XilverMoods - By @ytmgtwira420'
             ...
             let sourceUrl =  'https://instagram.com/ytmgtwira420'
             let mediaUrl = 'https://instagram.com/ytmgtwira420'                            
